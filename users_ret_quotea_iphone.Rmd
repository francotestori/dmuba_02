...
length(is.na(top_tweets_by_retweet_count$cat_verified))
```
...
table(top_tweets_by_retweet_count$cat_pais)
```
...
top_tweets_by_retweet_count=top_tweets_by_retweet_count[top_tweets_by_retweet_count$source=='Twitter for iPhone',]
```
...
```{r}
length(unique(df_tweets_tuples$user_id))
```
...
status_user=top_tweets_by_retweet_count[,c("status_id","user_id")]
vocabulario_dia= inner_join(vocabulario_dia,status_user, c('status_id'='status_id') )
...
df_tuples=tupla[,c("user_id","item")]
...
```{r}
# reglas
trans2 <- as(split(df_tuples$item, df_tuples$user_id), "transactions")
print(trans2)
rules = apriori(trans2, parameter=list(target="rule", support=0.01, confidence=0.2, minlen=3, maxlen=5), appearance = list(rhs = 'statuses=muchos'))
#, appearance = list(rhs = 'rt=muchos')
print(rules)
View(inspect(sort(rules, by="lift", decreasing = TRUE)[1:20]))
```

```{r}
s=subset(rules, subset = !(lhs  %in% c("favorite=muchos","followers=muchos",'plataforma=Twitter for iPhone','verified=si')))
View(inspect(sort(s, by = "lift", decreasing = TRUE)))

s=subset(rules, subset = rhs  %ain% c("favorite=muchos", "retweet=muchos"))
View(inspect(sort(s, by = "lift", decreasing = TRUE)))
